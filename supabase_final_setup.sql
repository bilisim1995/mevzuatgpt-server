-- MevzuatGPT Supabase Final Setup
...
-- 2. Mevcut policy ve tabloları temizle
...
DROP POLICY IF EXISTS "public_can_view_completed_docs" ON public.mevzuat_documents;
DROP POLICY IF EXISTS "admins_can_manage_all_docs" ON public.mevzuat_documents;
DROP POLICY IF EXISTS "public_can_search_embeddings" ON public.mevzuat_embeddings;
DROP POLICY IF EXISTS "admins_can_manage_embeddings" ON public.mevzuat_embeddings;
DROP POLICY IF EXISTS "users_can_view_own_searches" ON public.search_logs;
DROP POLICY IF EXISTS "public_can_insert_search_logs" ON public.search_logs;
DROP POLICY IF EXISTS "admins_can_view_all_searches" ON public.search_logs;
...
-- 5. Embeddings tablosu (1536 dimension)
...
-- 7. Performance indexes (önce tablolar, sonra indexler)
...
CREATE INDEX idx_documents_uploaded_by ON public.mevzuat_documents(uploaded_by);
...
-- 8. Vector index (en son, tablo oluştuktan sonra)
CREATE INDEX idx_embeddings_vector ON public.mevzuat_embeddings 
...
CREATE POLICY "public_view_completed_docs" ON public.mevzuat_documents
    FOR SELECT USING (status = 'completed');
    
CREATE POLICY "admins_manage_docs" ON public.mevzuat_documents
    FOR ALL USING (
        EXISTS (SELECT 1 FROM public.user_profiles WHERE id = auth.uid() AND role = 'admin')
    );

CREATE POLICY "public_search_embeddings" ON public.mevzuat_embeddings
    FOR SELECT USING (
        EXISTS (SELECT 1 FROM public.mevzuat_documents WHERE id = document_id AND status = 'completed')
    );
    
CREATE POLICY "admins_manage_embeddings" ON public.mevzuat_embeddings
    FOR ALL USING (
        EXISTS (SELECT 1 FROM public.user_profiles WHERE id = auth.uid() AND role = 'admin')
    );

CREATE POLICY "users_view_own_searches" ON public.search_logs
    FOR SELECT USING (auth.uid() = user_id);
    
CREATE POLICY "public_insert_searches" ON public.search_logs
    FOR INSERT WITH CHECK (true);
    
CREATE POLICY "admins_view_all_searches" ON public.search_logs
    FOR ALL USING (
        EXISTS (SELECT 1 FROM public.user_profiles WHERE id = auth.uid() AND role = 'admin')
    );

-- 11. Trigger fonksiyonları
