...
#### Bulk Upload with Per-Document Task Tracking (October 19, 2025)
Enterprise-grade bulk PDF processing with individual task management:
- **Per-Document Tasks**: Each PDF receives unique Celery task_id for independent progress tracking
- **Batch Grouping**: Documents grouped by batch_id for aggregate progress monitoring
- **Partial Failure Handling**: Failed task enqueues receive synthetic task_id and immediate failure status to maintain batch consistency
- **Real-time Progress Tracking**: Redis-based progress with two monitoring levels:
  - **Individual Task**: `GET /api/admin/documents/bulk-upload/progress/{task_id}` 
  - **Batch Aggregate**: `GET /api/admin/documents/bulk-upload/batch/{batch_id}/progress`
- **Redis Architecture**: 
